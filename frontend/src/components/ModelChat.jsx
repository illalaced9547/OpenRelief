...
export default function ModelChat({context,open,onOpen,onClose,hidden=false}){
 ...
 const send=async(question=draft)=>{
  ...
  try{
   ...
   setMessages(m=>[...m,{role:'assistant',text:modelAnswer(text,prediction),context:label,source:live?'LIVE INFERENCE':'SAVED PREDICTION'}]);
 ...
 <section id="openrelief-chat" className={`model-chat panel-chat ${open&&!hidden?'chat-visible':''}`} inert={!open||hidden} aria-hidden={!open||hidden} role="dialog" aria-label="Ask about the data" onKeyDown={e=>{if(e.key==='Escape'){e.stopPropagation();close()}}}>
  ...
  <div className="chat-messages" role="log" aria-live="polite">{!messages.length&&<div className="chat-welcome"><h3>Understand the prediction.</h3><p>{stored?'Explore the model’s phase, rationale and source limitations for this historical sample.':'Choose a country with model data to explore its prediction.'}</p>{stored&&<div className="chat-prompts">{['Why this prediction?','How confident is it?','What are the sources?'].map(q=><button disabled={pending} key={q} onClick={()=>send(q)}>{q}<ArrowUp size={13}/></button>)}</div>}</div>}{messages.map((m,i)=><div className={`chat-message ${m.role}`} key={i}><small>{m.role==='user'?'YOU':m.source} · {m.context}</small><p>{m.text}</p></div>)}{pending&&<div className="chat-message assistant"><small>RUNNING INFERENCE</small><p>Requesting the historical sample’s prediction…</p></div>}<div ref={end}/></div>
  <div className="chat-disclaimer">{canGoLive?'Country-based inference · Saved output if unavailable · Answers formatted locally':stored?`Saved output · ${formatDate(snapshotMeta.generatedAt)} · Answers formatted locally`:'No model output for this country'}</div>

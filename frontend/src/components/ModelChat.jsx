...
import {PHASE_LABEL,liveFor,SNAPSHOT_META} from '../lib/liveModel';

const snapshotDate = SNAPSHOT_META.generatedAt?.slice(0,16).replace('T',' ')+' UTC';

function modelAnswer(question,live){
 ...
 if(/source|where.*data|evidence|provenance/.test(q))return `Forecast for ${live.country}: cutoff ${live.cutoff}, target month ${live.target_month}, held-out test sample ${live.sample_id}. This is one of ${live.num_examples_for_country} prepared test examples for this country, from a batch snapshot generated ${snapshotDate}.`;

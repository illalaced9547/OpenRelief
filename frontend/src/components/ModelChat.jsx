...
export default function ModelChat({context,open,onOpen,onClose,hidden=false}){
 ...
 const send=async(question=draft)=>{
  ...
  try{
   ...
   let answer,source;
   try{answer=await askOpenAI(text,{...prediction,phase_label:PHASE_LABEL[prediction.predicted_phase]},selected.country,messages.map(m=>({role:m.role,content:m.text})));source='OPENAI ANSWER'}
   catch(error){console.warn('OpenAI chat unavailable',error);answer=modelAnswer(text,prediction);source=live?'LIVE INFERENCE · LOCAL FALLBACK':'SAVED PREDICTION · LOCAL FALLBACK'}
 ...
 <section id="openrelief-chat" className={`model-chat panel-chat ${open&&!hidden?'chat-visible':''}`} inert={!open||hidden} aria-hidden={!open||hidden} role="dialog" aria-label="Ask about the data" onKeyDown={e=>{if(e.key==='Escape'){e.stopPropagation();close()}}}>
  ...
  <div className="chat-disclaimer">{canGoLive?'OpenAI answers grounded in model output · Nebius inference · Saved output if unavailable':stored?`OpenAI answers grounded in saved output · ${formatDate(snapshotMeta.generatedAt)}`:'No model output for this country'}</div>

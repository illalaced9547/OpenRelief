...
function App(){
 ...
 const showHover=(e,c)=>{if(drag.current)return;const d=forecasts[c.id];setHovered({name:c.properties.name,risk:d?riskFor(d):null,live:!!liveFor(d?.iso3),x:Math.min(e.clientX+20,window.innerWidth-170),y:Math.min(e.clientY+20,window.innerHeight-80)})};
 ...
 return <div className="app-shell dark-dashboard">
  ...
  {hovered&&<div className="country-tooltip" style={{left:hovered.x,top:hovered.y}}><span>{hovered.name}</span><strong style={{color:hovered.risk!==null?color(hovered.risk):'#aaa'}}>{hovered.risk!==null?`${hovered.risk}%`:'No data'}</strong><small>{hovered.live?'MODEL FORECAST':'DEMO RISK'}</small></div>}

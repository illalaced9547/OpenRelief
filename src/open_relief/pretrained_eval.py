"""Pretrained-baseline (no fine-tuning) evaluation on the full test partition,
sharded so multiple Nebius jobs can run the same generation workload in parallel.

Needed because every existing run used --skip-pretrain-eval: there is no
pretrained-vs-fine-tuned comparison anywhere yet, on any cohort size. This
evaluates the untouched official checkpoint only - no LoRA, no training.
"""
import argparse
import json
...
def main():
    ...
    model.eval()
    print(f"Pretrained checkpoint loaded: {a.checkpoint}@{checkpoint_sha[:12]}", flush=True)
